...
class IPAdapter(torch.nn.Module):
    ...
    def load_from_checkpoint(self, ckpt_path: str):
        ...
        proj_path = os.path.join(ckpt_path, "proj_model.pt")
...
def monkey_patch_sd15_pipeline_for_ipadapter(pipe):
    # Install IP attention processors on the UNet so condition_embeds are consumed
    # _ = init_adapter(pipe.unet)
    ...
    def __call_with_ipadapter__(
        ...
        if positive_condition is not None:
            if do_classifier_free_guidance:
                ...
                half = prompt_embeds.shape[0]
            cond_embeds = self._encode_prompt(
                [positive_condition] * half,
                device,
                num_images_per_prompt,
                do_classifier_free_guidance,
